<?php
$characters="qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890";function encodeText($code,$key){$enbs=base64_encode($code);$tractadd=explode("=",$enbs);$result=str_repeat("//e//lai//na//",count($tractadd)-1);
$enbs=str_replace("=",$result,$enbs);$diff=explode("//e//lai//na//",$enbs);$genarr=genArrCode($key);$enCode="";for($i=0;$i<=strlen($diff[0])-1;$i++){$enCode.=$genarr[$diff[0][$i]]."\/\/\/";}global$lastIndex;$enCode.=$lastIndex;return base64_encode("//-----------//".base64_encode($key)."//----------//".base64_encode($enCode));}
